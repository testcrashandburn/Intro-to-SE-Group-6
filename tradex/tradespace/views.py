...
def history(request):
	if request.user.is_authenticated:
		customer = request.user
		historylist = [Order.objects.values_list(), Order.customer.field.get_attname()]
	else:
		historylist = {'get_cart_total':0, 'get_cart_items':0, "shipping":False}
...
def product_list(request):
    ...
    return render(request, 'tradespace/product_list.html', {'products': products})

def order_history(request):
    if request.user.is_authenticated:
        orders = Order.objects.filter(customer=request.user)
        return render(request, 'tradespace/order_history.html', {'orders': orders})
    else:
...
def compare(request):
	...
	return render(request, 'tradespace/compare.html', {'product1': product1, 'product2': product2})


def sellerhome(request):
    if request.user.is_authenticated:
        products = Product.objects.filter(creator=request.user)
        return render(request, 'tradespace/sellerhome.html', {'products': products})
    else:
        return redirect('login')  
